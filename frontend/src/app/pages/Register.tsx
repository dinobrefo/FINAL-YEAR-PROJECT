    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#06111F] text-slate-900 dark:text-[#F8FAFC] flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans transition-colors duration-200">
      ...
      <div className="mb-6 flex items-center gap-3 relative z-10">
        ...
        <div>
          ...
          <p className="text-xs text-red-600 dark:text-red-400 font-mono font-semibold">HEALTHCARE STAFF ENROLLMENT</p>
        </div>
      </div>
      ...
      <Card className="w-full max-w-lg shadow-2xl bg-white/95 dark:bg-[#111C2D]/95 border border-slate-200 dark:border-white/10 rounded-2xl relative z-10 backdrop-blur-xl">
        <CardHeader className="text-center">
          <CardTitle className="text-xl text-slate-900 dark:text-white font-mono">Create Portal Account</CardTitle>
          <CardDescription className="text-slate-500 dark:text-slate-400">Register for role-based emergency access & hospital coordination</CardDescription>
        </CardHeader>
        <CardContent>
          {success ? (
            <div className="p-6 text-center space-y-3 font-mono">
              <CheckCircle className="h-12 w-12 text-emerald-500 dark:text-emerald-400 mx-auto animate-bounce" />
            ...
            <form onSubmit={handleSubmit} className="space-y-4 font-mono text-xs">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full p-2.5 border border-slate-300 dark:border-white/10 rounded-xl bg-slate-50 dark:bg-[#081827] text-slate-900 dark:text-white text-sm font-sans focus:ring-2 focus:ring-red-500 focus:border-red-500/50 focus:outline-none"
                  />
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-bold uppercase text-slate-700 dark:text-slate-300">Phone Number</label>
                  <input
                    type="tel"
                    value={phone}
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase text-slate-700 dark:text-slate-300">Work Email Address</label>
                <input
                  type="email"
                  value={email}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold uppercase text-slate-700 dark:text-slate-300">System Role</label>
                  <select
                    value={role}
                    onChange={(e) => setRole(e.target.value)}
                    className="w-full p-2.5 border border-slate-300 dark:border-white/10 rounded-xl bg-slate-50 dark:bg-[#081827] text-slate-900 dark:text-white text-sm font-sans cursor-pointer focus:ring-2 focus:ring-red-500 focus:border-red-500/50 focus:outline-none"
                  >
                ...
                {(role === 'hospital' || role === 'doctor' || role === 'nurse') && (
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold uppercase text-slate-700 dark:text-slate-300">Affiliated Facility</label>
                    <select
                      value={hospitalId}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold uppercase text-slate-700 dark:text-slate-300">Confirm Password</label>
                  <input
                    type="password"
              ...
              <p className="text-center text-xs text-slate-500 dark:text-slate-400 pt-2 font-sans">
                Already registered?{' '}
