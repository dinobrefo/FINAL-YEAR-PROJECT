    ...
    <div className="fixed bottom-4 right-4 z-[9999] font-sans">
      {!isOpen ? (
        <button
          ...
          className="group flex items-center gap-2.5 px-4 py-2.5 rounded-full bg-white/95 dark:bg-slate-900/95 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-900 dark:text-white border-2 border-teal-500/60 shadow-2xl backdrop-blur-md transition-all hover:scale-105 cursor-pointer"
        >
          <div className="relative">
            <GraduationCap className="h-5 w-5 text-teal-600 dark:text-teal-400 group-hover:rotate-12 transition-transform" />
            <span className="absolute -top-1 -right-1 h-2 w-2 rounded-full bg-teal-500 dark:bg-teal-400 animate-ping" />
          </div>
          <span className="text-xs font-bold tracking-wide">Examiner Defense Tour</span>
          <span className="text-[10px] uppercase font-mono px-1.5 py-0.2 rounded bg-teal-500/15 dark:bg-teal-500/20 text-teal-700 dark:text-teal-300 border border-teal-500/40">
            A+ Demo
          </span>
          <ChevronUp className="h-4 w-4 text-slate-400" />
        </button>
      ) : (
        <div className="w-[390px] max-h-[85vh] bg-white/95 dark:bg-slate-950/95 backdrop-blur-xl border-2 border-teal-500/60 rounded-2xl shadow-2xl overflow-hidden flex flex-col text-slate-900 dark:text-slate-100 animate-in fade-in slide-in-from-bottom-4 duration-200">
          {/* Header */}
          <div className="p-3.5 bg-gradient-to-r from-slate-100 to-slate-200 dark:from-slate-900 dark:to-slate-950 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="p-1.5 rounded-lg bg-teal-500/15 dark:bg-teal-500/20 border border-teal-500/40">
                <GraduationCap className="h-4 w-4 text-teal-600 dark:text-teal-400" />
              </div>
              <div>
                <h4 className="text-xs font-bold text-slate-900 dark:text-white flex items-center gap-1.5">
                  Capstone Defense Controller
                  <span className="text-[9px] px-1.5 py-0.2 rounded-full bg-emerald-500/15 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 font-mono">
                    LIVE
                  </span>
                </h4>
                <p className="text-[10px] text-slate-500 dark:text-slate-400">1-click demonstration scenarios for examiners</p>
              </div>
            </div>
            <button
              onClick={() => setIsOpen(false)}
              className="p-1 rounded-md text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-slate-800 text-xs cursor-pointer"
            >
          ...
          <div className="p-2.5 bg-slate-50 dark:bg-slate-900/60 border-b border-slate-200 dark:border-slate-800/80">
            <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-1.5 flex items-center gap-1">
              <UserCheck className="h-3 w-3 text-teal-600 dark:text-teal-400" /> 1-Click Role Switcher
            </p>
                ...
                return (
                  <button
                    ...
                    className={`px-2 py-1.5 rounded-lg text-[11px] font-semibold flex items-center justify-center gap-1.5 border transition-all cursor-pointer ${
                      isActive
                        ? "bg-teal-500/20 border-teal-500/60 text-teal-700 dark:text-teal-300 shadow-sm"
                        : "bg-white dark:bg-slate-900/80 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800"
                    }`}
                  >
          ...
          <div className="p-3 space-y-2 overflow-y-auto max-h-[46vh]">
            <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 flex items-center justify-between">
              ...
              <span className="text-[9px] text-teal-600 dark:text-teal-400">4 Scenarios Ready</span>
            </p>
              ...
              return (
                <div
                  ...
                  className={`p-2.5 rounded-xl border transition-all ${
                    isSelected
                      ? "bg-teal-50 dark:bg-teal-950/40 border-teal-500/60 shadow-lg"
                ...
                >
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <div>
                      ...
                      <span className="text-[9px] font-mono text-teal-600 dark:text-teal-300">{sc.badge}</span>
                    </div>
                    <button
                  ...
                  <p className="text-[10px] text-slate-600 dark:text-slate-300 leading-snug">{sc.description}</p>
                </div>
              );
            })}
          </div>
          ...
          {/* Footer Controls */}
          <div className="p-2.5 bg-slate-100 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
            <button
            ...
            <span className="text-[10px] font-mono text-slate-400 dark:text-slate-500">KNUST Final Year Capstone</span>
          </div>
        </div>
      )}
    </div>
